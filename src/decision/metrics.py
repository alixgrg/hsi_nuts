...
def binary_detection_metrics(
    ...
    non_target_class: str = "non_target",
) -> dict:
    """
    ...
    if target_class == "peanut":
        out["peanut_sensitivity"] = sens
        out["almond_specificity"] = spec
    return out
...
def add_binary_confusion_case(
    ...
    if true_col is None:
        true_col = f"true_{target_class}_{level}"

    if pred_col is None:
        pred_col = f"predicted_{target_class}_{level}"
