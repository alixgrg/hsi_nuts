...
def _preprocessing_step_count(value: Any) -> int:
    ...
    if not text or text.lower() in {"raw", "none", "nan", "<na>"}:
        return 0
    try:
    ...
    for separator in ("+", "|", ",", ";"):
        if separator in text:
            return len([token for token in text.split(separator) if token.strip()])
    return 1


def select_final_models_lexicographic(
    candidate_summary_df: pd.DataFrame,
    *,
    expected_tracks: Sequence[str] = expcfg.SIMCA_EVALUATION_TRACKS,
    priority: Sequence[str] = expcfg.SIMCA_FINAL_LEXICOGRAPHIC_PRIORITY,
    tolerances: Mapping[str, float] = expcfg.SIMCA_FINAL_EQUIVALENCE_TOLERANCES,
) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Select at most one primary model per track after hard safety checks.

    The procedure is deterministic and strictly lexicographic. It applies no
    scalar score, family quota, diversity rule, top-N cap or cross-track
    deduplication. A track may legitimately end without an acceptable model.
    """
    if candidate_summary_df is None or len(candidate_summary_df) == 0:
        raise ValueError("candidate_summary_df is empty.")
    pool = candidate_summary_df.copy()
    required = {
        "calibration_id",
        "evaluation_track",
        "track_id",
    ...
    if missing:
        raise KeyError(f"Missing final-selection columns: {missing}")
    forbidden = sorted(
        set(pool.columns).intersection(expcfg.ACTIVE_PROTOCOL_FORBIDDEN_SCORE_COLUMNS)
    )
    if forbidden:
        raise ValueError(f"Score columns are forbidden in final selection: {forbidden}")
    pool["preprocessing_step_count"] = pool.get(
        "preprocessing_steps", pd.Series(pd.NA, index=pool.index)
    ).map(_preprocessing_step_count)
    pool["runtime_seconds"] = pd.to_numeric(
        pool.get("runtime_seconds", pd.Series(np.nan, index=pool.index)),
        errors="coerce",
    )
    pool["selection_role"] = "not_selected"
    pool["selection_reason"] = ""
    pool["is_final_selected"] = False

    metric_columns = {
        "target_miss_rate": ("safety_target_miss_rate", "min"),
        "worst_image_target_miss_rate": (
            "worst_image_target_miss_rate", "min",
        ),
        "worst_seed_target_miss_rate": (
            "worst_seed_target_miss_rate", "min",
        ),
        "smallest_fragment_recall": ("smallest_fragment_recall", "max"),
        "false_accept_rate": ("safety_false_accept_rate", "min"),
        "uncertain_rate": ("safety_uncertain_rate", "min"),
        "stability_status": ("_stability_rank", "min"),
        "preprocessing_step_count": ("preprocessing_step_count", "min"),
        "n_components": ("n_components", "min"),
        "runtime_seconds": ("runtime_seconds", "min"),
    }
    pool["_stability_rank"] = pool.get(
        "stability_status", pd.Series("not_estimable", index=pool.index)
    ).astype(str).map(
        {
            "robust": 0,
            "not_applicable_deterministic": 0,
            "robust_with_warning": 1,
        }
    ).fillna(2)
    selected_indices: list[int] = []
    ...
    for track in expected_tracks:
        track_indices = pool.index[
            pool["evaluation_track"].astype(str).eq(str(track))
        ]
        group = pool.loc[track_indices].copy()
        if group.empty:
            protocol_rows.append(
                {
                    "evaluation_track": track,
                    "track_id": expcfg.SIMCA_EVALUATION_TRACK_IDS.get(track, ""),
                    "track_status": "not_estimable",
                    "selected_calibration_id": "",
                    "n_units": 0,
                    "n_eligible": 0,
                    "selection_method": "hard_guardrails_then_lexicographic",
                }
            )
            continue
        eligibility_values = set(group.get("eligibility_status", "").astype(str))
        unsupported = next(
            (
                value
                for value in (
                ...
                if value in eligibility_values
            ),
            None,
        )
        eligible = group.loc[
            group["final_candidate_status"].astype(str).eq(
                "eligible_for_final_selection"
            )
        ].copy()
        if eligible.empty:
            track_status = unsupported or "no_acceptable_candidate"
            protocol_rows.append(
                {
                    "evaluation_track": track,
        ...
        for priority_name in priority:
            column, direction = metric_columns[priority_name]
            if column not in survivors:
                continue
            values = pd.to_numeric(survivors[column], errors="coerce")
            finite = values.notna() & np.isfinite(values)
            if not finite.any():
                continue
            # Missing values never beat finite values, but a metric inapplicable
            # to every survivor is skipped (e.g. fragment recall for object tracks).
            survivors = survivors.loc[finite].copy()
            values = pd.to_numeric(survivors[column], errors="coerce")
            best = float(values.max() if direction == "max" else values.min())
            tolerance_key = priority_name
            tolerance = float(tolerances.get(tolerance_key, 0.0))
            if priority_name == "runtime_seconds" and best > 0:
                keep = values.le(best * (1.0 + tolerance))
            elif direction == "max":
                keep = values.ge(best - tolerance)
            else:
                keep = values.le(best + tolerance)
            decisions.append(
                f"{priority_name}:{direction}:best={best:.6g}:tol={tolerance:.6g}"
            )
            survivors = survivors.loc[keep].copy()
            if len(survivors) == 1:
                break

        survivors = survivors.sort_values("calibration_id", kind="mergesort")
        primary_index = int(survivors.index[0])
        selected_indices.append(primary_index)
        locked_indices.extend(map(int, survivors.index))
        pool.loc[primary_index, "selection_role"] = "primary"
        pool.loc[primary_index, "is_final_selected"] = True
        pool.loc[primary_index, "selection_reason"] = ";".join(decisions)
        alternative_indices = survivors.index.difference([primary_index])
        pool.loc[alternative_indices, "selection_role"] = "equivalent_alternative"
        pool.loc[alternative_indices, "selection_reason"] = (
            "within_all_lexicographic_equivalence_tolerances"
        )
        protocol_rows.append(
            {
                "evaluation_track": track,
                ...
                "selected_calibration_id": str(pool.loc[primary_index, "calibration_id"]),
                "n_units": int(len(group)),
                "n_eligible": int(len(eligible)),
                "n_equivalent_alternatives": int(len(alternative_indices)),
                "selection_method": "hard_guardrails_then_lexicographic",
            }
        )

    selected = pool.loc[selected_indices].drop(columns="_stability_rank").copy()
    locked = pool.loc[sorted(set(locked_indices))].drop(columns="_stability_rank").copy()
    annotated = pool.drop(columns="_stability_rank").copy()
    protocol = pd.DataFrame(protocol_rows)
    selected["final_rank_in_track"] = 1
    return (
        selected.reset_index(drop=True),
        locked.reset_index(drop=True),
        protocol.reset_index(drop=True),
    )
