...
from src.workflows.simca_cv_calibration import refit_empirical_cv_rule_row
...
#Delete in future version, keeping for backward compatibility
def make_peanut_train_filters(train_batches=None, split=None) -> dict[str, Any]:
    """Backward-compatible wrapper."""
    return make_target_train_filters(
        target_class="peanut",
...
def refit_selected_simca_configs(
    ...
    for _, row in selected_configs_df.iterrows():
        ...
        try:
            ...
            target_object_true_col = f"true_{target_class}_object"
            target_object_pred_col = f"predicted_{target_class}_object"
            ...
            target_pixel_true_col = f"true_{target_class}_pixel"
            target_pixel_pred_col = f"predicted_{target_class}_pixel"
