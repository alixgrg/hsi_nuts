...
from src.decision.labels import DEFAULT_TARGET_CLASS, DEFAULT_NON_TARGET_LABEL, predicted_col
...
# def _metrics_by_batch(
#     obj_df: pd.DataFrame,
#     target_class: str = "peanut",
#     non_target_class: str | None = None,
# ) -> dict:
#     """
...
#     - batch3_target_sensitivity
#     - batch3_non_target_specificity
#     - batch3_fn
#     - batch3_fp
#     """
...
#     if non_target_class is None:
#         non_target_class = f"non_{target_class}"

#     true_col = f"true_{target_class}_object"
#     pred_col = f"predicted_{target_class}_object"

#     if true_col not in obj_df.columns and "true_peanut_object" in obj_df.columns:
#         true_col = "true_peanut_object"
#     if pred_col not in obj_df.columns and "predicted_peanut_object" in obj_df.columns:
...
#     if sens_col not in batch_df.columns and "peanut_sensitivity" in batch_df.columns:
#         sens_col = "peanut_sensitivity"
#     if spec_col not in batch_df.columns and "almond_specificity" in batch_df.columns:
#         spec_col = "almond_specificity"

#     out = {
#         "min_batch_balanced_accuracy": float(batch_df["balanced_accuracy"].min()),
#         "min_batch_target_sensitivity": float(batch_df[sens_col].min()),
#         "min_batch_non_target_specificity": float(batch_df[spec_col].min()),
#     }
...
#         out[f"{prefix}_target_sensitivity"] = float(row[sens_col])
#         out[f"{prefix}_non_target_specificity"] = float(row[spec_col])
